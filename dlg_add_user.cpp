...
#include "ui_dlg_add_user.h"
#include<QFile>
dlg_add_user::dlg_add_user(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::dlg_add_user)
{
    ui->setupUi(this);
...
dlg_add_user::~dlg_add_user()
{
    delete ui;
}

void dlg_add_user::setType(bool isAdd, UserInfo info)
{
    m_isAdd=isAdd;
    m_info=info;
...
void dlg_add_user::on_pushButton_2_clicked()
{
    this->hide();
}
/**
  确定按钮
 * @brief dlg_add_user::on_pushButton_clicked
 */
void dlg_add_user::on_pushButton_clicked()
{
    qDebug() << "click OK 001";
    UserInfo info;
    auto ptr=sonSql::getinstance();
